...
namespace CATALOGO
{
    public partial class frmLista_Familias : Form
    {
        private bool _Salir;
        ...
        private void Refrescar_Grid()
        {
            try
            {
                ...
                List<tbFamilias> _Datos = _DTFamilias;
                if (txtCodigo.Text != "")
                {
                    _Datos = _DTFamilias.Where(x => x.Familia_Id == Convert.ToString(txtCodigo.Text)).ToList();
                }
                if (txtNombre.Text != "")
                {
                    _Datos = _DTFamilias.Where(x => x.Nombre == Convert.ToString(txtNombre.Text)).ToList();
                }
                dtgGrid.Rows.Clear();
                ...
                if (_Datos != null)
                {
                    if (_Datos.Count > 0)
                    {
                        ...
                        MessageBox.Show("No se encontraron Familias", "Familias", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show("No se encontraron Familias", "Familias", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            {
                MessageBox.Show("Problemas al cargar la información", "Familias", MessageBoxButtons.OK, MessageBoxIcon.Error);
                this.dtgGrid.Refresh();
            }
        }
        ...
        private void Eliminar_Familia()
        {
            try
            {
                if (dtgGrid.SelectedRows.Count > 0)
                {
                    ...
                    tbFamilias pro = new tbFamilias();

                    pro.Familia_Id = row.Cells[_clmCodigo].Value.ToString();

                    if (MessageBox.Show("Esta seguro que quiere eliminar la familia", "Familias", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        if (_Trastienda.WebApiProductos.EliminarFamilia(pro.Familia_Id))
                            MessageBox.Show("Se elimino la familia correctamente", "Familias", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        else
                            MessageBox.Show("Se produjo un error al eliminar la familia", "Familias", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    Buscar();
                }
                else
                {
                    MessageBox.Show("Bede seleccionar una familia", "Lista Familias", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Se produjo un error al eliminar la familia" + "\n" + ex.Message, "Lista Familias", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void Bn_Agregar_Click(object sender, EventArgs e)
        {
            ...
            if (frm.Execute(_Trastienda, new tbFamilias()))
                Refrescar_Grid();
            else
                Refrescar_Grid();
        }
        ...
        private void frmLista_Familias_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.F1:
        ...
        private void frmLista_Familias_FormClosed(object sender, FormClosedEventArgs e)
        {
            _Salir = true;
        }
    }
